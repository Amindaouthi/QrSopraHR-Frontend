  ...
  useEffect(() => {
    ...
    if (userDataString) {
      try {
        ...
        if (matricul) {
          axios
            .get(`http://localhost:8083/api/user/${matricul}`)
            .then((response) => {
      ...
      >
        ...
        {!isAdmin && [
          ...
          <MenuItem key="new-link" onClick={handleNavigateToStats}>
            ...
            <ListItemText>{t('My Statis')}</ListItemText>
          </MenuItem>,
        ]}
