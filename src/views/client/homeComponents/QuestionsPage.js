...
const QuestionsPage = () => {
  ...
  const handleTrashClick = async (questionId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:8082/api/questions/${questionId}`);
        fetchAllQuestions();
  ...
  try {
    ...
    if (isQuestionFavorite) {
      ...
      const response = await axios.delete(
        `http://localhost:8082/api/favorites/${favoriteId}`,
        {
          headers: {
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <>
            ...
            >
            <FilterTitle> {t('make your search easy :')}</FilterTitle>
